...
class RMPSoftMaxVel2Metric(Scene):
    CONFIG = {
        "plane_kwargs": {
            "color": RED_B
        },
        "goal_loc": 1.75 * RIGHT - 0.5 * UP,
        "start_loc": 2.5 * LEFT - 1.5 * UP
    }
    ...
    def construct(self):
        ...
        formula = TexMobject("f_{1}(x, \dot{x}) = \\alpha\\ s(x_{g} -x) - \\beta \dot{x}")
        formula.bg=BackgroundRectangle(formula,fill_opacity=1)
        metric = TexMobject("A_{1} = diag(1,0)")
        ...
        formula2 = TexMobject("f_{2}(x, \dot{x}) = \\alpha\\ s(x_{g} -x)")
        formula2.bg = BackgroundRectangle(formula2, fill_opacity=1)
        metric2 = TexMobject("A_{2} = diag(0,1)")
        metric2.bg = BackgroundRectangle(formula2, fill_opacity=1)
        formula2_group = VGroup(formula2.bg, formula2)  # Order matters
        formula2_group.next_to(metric_group, DOWN)
        metric2_group = VGroup(metric2.bg, metric2)  # Order matters

        metric2_group.next_to(formula2_group, DOWN)
        ...
        self.play(ShowCreation(formula2_group))
        self.add_foreground_mobject(formula2_group)

        self.play(ShowCreation(metric2_group))
        self.add_foreground_mobject(metric2_group)
        self.wait(5)
